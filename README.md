...
GIF動画
```
GIF動画もまた、静止画像の連続で表現される為、変換可能である。
但し、変換文字群を動画として再生する何らかの手法が必要である。
```
最も近いという判断
```
将棋やチェスに見られるような各マス目に重み付けし、その最大値を最も近いと見なす。
この最大値は、特徴に対して数値を高く各マスに設定する為、最大値が結果として、ある画像プロファイルに最も類似である。
比較元の画像プロファイルの数値は、限界とイコールになる。比較元と比較先の画像プロファイルが同値の場合は、完全に一致した画像となる。
```
